...
class CommsManager:
    def __init__(self):
        ...
        # ZMQ sockets are not thread-safe: _command_tx and _heartbeat_tx both
        # send on _cmd_pub from separate threads, so every send must be
        # serialized or libzmq's internal refcounting gets corrupted
        # (observed as "Assertion failed: refs_ >= 0" native crashes).
